...
function New-Regex.SplitBySegmentSize {
    ...
    .NOTES
        also warning, regex appears to match *chars* not codepoints. at least this one.
    ...
    param(
        ...
        [ArgumentCompletions('Original', 'MultiLine')]
...
function FancyName {
    ...
    process {
        ...
        | Join-String -sep $Separator -p {
            $_ | FancySegment -SegmentLength $Len -Delim $Delim
        }
    }
}
...
New-Regex.SplitBySegmentSize @Splat -SegmentLength 4 -InputText $_
    | Join-String -op $c2
    | Join-String -op $c3 -os $PSStyle.Reset }
